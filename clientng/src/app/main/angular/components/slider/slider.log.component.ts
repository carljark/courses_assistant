...
$('#sliderDia').on('touchstart mousedown', e => {
    ...
    $('body').on('touchmove mousemove', ev => {
        ...
        $('body').off('touchmove mousemove');
        checkDevolver(parseInt($('#ctd').val() as string, 10));
    });
});


$('#ctdDia').on('change', function (e) {
    const valor = parseInt($(this).val() as string, 10);
    let posX: number;
    if (valor >= minDia && valor <= limiteDia) {
        posX = $('#reglaDia').outerWidth() * valor / limiteDia;
